...
"""Continual Learning (CL) training of PI05 on LIBERO benchmark.
...
Usage examples:
  ...
  python train_pi05_cl_libero.py \\
      --pretrained lerobot/pi05_base \\
      --dataset_repo_id lerobot/libero_10 \\
      --output_dir outputs/tail_pi05_libero10 \\
...
Multi-GPU examples (DDP via torchrun):
  ...
  # 4-GPU DDP (TAIL):
  torchrun --nproc_per_node=4 train_pi05_cl_libero.py --cl_method tail --use_lora ...

  # 2-GPU DDP (sequential):
  torchrun --nproc_per_node=2 train_pi05_cl_libero.py --cl_method sequential ...

  # Single GPU:
  python train_pi05_cl_libero.py --gpus 3 --cl_method tail --use_lora ...
"""
...
class _PI05LoRABridge(torch.nn.Module):
    """Bridge between PeftModel and PI05Policy.

    PeftModelForFeatureExtraction.forward binds the first positional arg to
    `input_ids`. This wrapper accepts that and routes it to PI05Policy.forward(batch).
    """
    ...
    def forward(self, input_ids=None, **kwargs):
        ...
        raise TypeError(
            f"_PI05LoRABridge.forward: expected batch dict in 'input_ids', "
...
def load_lora_adapter(base_policy: PI05Policy, adapter_dir: Path) -> Any:
    _require_peft()
    from peft import PeftModel

    bridge = _PI05LoRABridge(base_policy)
    return PeftModel.from_pretrained(bridge, str(adapter_dir), is_trainable=False)


def merge_lora_into_policy(peft_model: Any) -> PI05Policy:
    """Merge LoRA weights into the base PI05Policy and return it."""
    merged_bridge: _PI05LoRABridge = peft_model.merge_and_unload()
    return merged_bridge.policy


def get_base_config(policy: Any) -> PI05Config:
    p = unwrap_dp(policy)
    if hasattr(p, "base_model"):
        return p.base_model.model.config
    if isinstance(p, _PI05LoRABridge):
        return p.config
    return p.config
...
def cache_pretrained_weights(pretrained: str) -> tuple[Any, dict]:
    ...
    base = PI05Policy.from_pretrained(pretrained)
    config = base.config
...
def load_base_policy(
    ...
    """Load PI05Policy with features adapted to the dataset.

    PI05Policy(config) does NOT download VLM weights — that only happens in
    ...
    if _cached is not None:
        ...
        log.info(f"  Loading pretrained PI05: {pretrained}")
        base = PI05Policy.from_pretrained(pretrained)
        config = base.config
    ...
    # Apply CLI overrides for PI05-specific training flags
    if args is not None:
...
def eval_sequential(
    ...
    for task_order, task_idx in enumerate(task_indices):
        ...
        policy = PI05Policy.from_pretrained(ckpt_path).to(device)
        policy.eval()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Continual Learning training of PI05 on LIBERO (sequential or TAIL)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # ── Model ──────────────────────────────────────────────────────
    p.add_argument("--pretrained", default="lerobot/pi05_base",
                   help="Initial PI05 checkpoint (HF Hub ID or local path).")
    ...
    p.add_argument("--lora_target_modules",
                   ...
                   help="Regex or comma-separated module names to apply LoRA to. "
                        "Default targets PI05 action expert attention + projection layers.")

    # ── Training ──────────────────────────────────────────────────
    p.add_argument("--output_dir", default="outputs/cl_pi05_libero")
    ...
    p.add_argument("--chunk_size", type=int, default=50,
                   help="Number of predicted action steps (PI05 default: 50).")
    p.add_argument("--n_obs_steps", type=int, default=1,
                   help="Observation history length (PI05 default: 1).")
    ...
    p.add_argument("--num_inference_steps", type=int, default=10,
                   help="Number of flow-matching denoising steps at inference time (PI05 default: 10).")
    ...
    p.add_argument("--wandb_project", default="cl-pi05-libero",
